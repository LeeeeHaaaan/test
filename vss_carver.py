...
def carve_data_block(disk_image, image_offset, volume_size, debug):
    ...
    while readinto_ctypes_struct(disk_image, store_block_header) and ((image_offset - base_offset) < volume_size):
        ...
        if (current_time - before_time).seconds >= 3:
            ...
            sys.stderr.write(
                '\r' + "Progress: {0} / {1} bytes ({2:.2%}) at {3}".format((image_offset - base_offset), volume_size,
                                                                           ((image_offset - base_offset) / volume_size),
        ...
        if store_block_header.vssid == vss_identifier and store_block_header.version == 1:
            ...
            if chunk_head_record_type == store_block_header.record_type:
                if store_block_header.next_block_offset - store_block_header.current_block_offset > 0x4000 or store_block_header.next_block_offset - store_block_header.current_block_offset < 0x0:
                    if debug:
                        print("{0}-{1}({2}) : Ver:{3} RType:{4} Next:{5}".format(hex(chunk_head),
                                                                                 hex(store_block_header.current_block_offset),
                                                                                 hex(store_block_header.current_block_offset - chunk_head + 0x4000),
                                                                                 store_block_header.version,
                                                                                 store_block_header.record_type,
                                                                                 hex(store_block_header.next_block_offset)))
                    chunk_head = 0
                    chunk_head_record_type = 0
                    chunk_continue = 1
                ...
                if debug:
                    print("{0}-{1}({2}) : Ver:{3} RType:{4}/{5} Next:{6}  Corrupt Chunk?".format(hex(chunk_head),
                                                                                                 hex(store_block_header.current_block_offset),
                                                                                                 hex(store_block_header.current_block_offset - chunk_head + 0x4000),
                                                                                                 store_block_header.version,
                                                                                                 chunk_head_record_type,
                                                                                                 store_block_header.record_type,
                                                                                                 hex(store_block_header.next_block_offset)))
                chunk_head = 0
                chunk_head_record_type = 0
...
def group_store_block(list_store_block_chunk, debug):
    ...
    if debug:
        ...
        for store in list_snapshot_set:
            print("header: {0}: RType:{1} Offset:{2}".format(hex(store['header'].head.current_block_offset),
                                                             store['header'].head.record_type,
                                                             [hex(x) for x in store['header'].list_next_block_offset]))
            print("block : {0}: RType:{1} Offset:{2}".format(hex(store['block'].head.current_block_offset),
                                                             store['block'].head.record_type,
                                                             [hex(x) for x in store['block'].list_next_block_offset]))
            print("range : {0}: RType:{1} Offset:{2}".format(hex(store['range'].head.current_block_offset),
                                                             store['range'].head.record_type,
                                                             [hex(x) for x in store['range'].list_next_block_offset]))
            print("curr  : {0}: RType:{1} Offset:{2}".format(hex(store['cur_bitmap'].head.current_block_offset),
                                                             store['cur_bitmap'].head.record_type, [hex(x) for x in
                                                                                                    store[
                                                                                                        'cur_bitmap'].list_next_block_offset]))
            print("prev  : {0}: RType:{1} Offset:{2}\n".format(hex(store['prev_bitmap'].head.current_block_offset),
                                                               store['prev_bitmap'].head.record_type, [hex(x) for x in
                                                                                                       store[
                                                                                                           'prev_bitmap'].list_next_block_offset]))

    return list_snapshot_set


def make_list_next_block_offset(dict_store_block, list_next_block_offset, next_block_offset, debug,
        ...
        if next_block_offset in dict_store_block:
            if debug:
                if next_block_offset in dict_referred_offset and len(dict_referred_offset[next_block_offset]) > 1:
                    print("Data block offset: {0} -> Next: {1}".format(hex(before_next_block_offset),
                                                                       hex(next_block_offset)))
                    print("Offset {0} is referred from multiple data blocks: {1}".format(hex(next_block_offset),
                                                                                         [hex(x) for x in
                                                                                          dict_referred_offset[
                                                                                              next_block_offset]]))
                ...
                if dict_store_block[next_block_offset].next_block_offset != 0x0:
                    dict_referred_offset[dict_store_block[next_block_offset].next_block_offset].append(
                        next_block_offset)
    ...
    for store in list_snapshot_set:
        for record_type in store.keys():
            ...
            if chunk.head.next_block_offset != 0x0:
                result = make_list_next_block_offset(dict_store_block, chunk.list_next_block_offset,
                                                     chunk.head.next_block_offset, debug, dict_referred_offset)

            if len(chunk.list_next_block_offset) >= 2 and dict_store_block[
                chunk.list_next_block_offset[-2]].next_block_offset != 0x0:
    ...
    if debug:
        ...
        for block_offset in dict_referred_offset:
            if len(dict_referred_offset[block_offset]) > 1:
                print("{0} is referred from {1}".format(hex(block_offset),
                                                        [hex(x) for x in dict_referred_offset[block_offset]]))
...
def deduplicate_catalog(dict_disk_catalog_entry, list_snapshot_set):
    ...
    for snapshot_set in list_snapshot_set:
        for store_guid in dict_disk_catalog_entry.keys():
            if dict_disk_catalog_entry[store_guid][1].store_header_offset == snapshot_set[
                'header'].head.current_block_offset:
            ...
            for next_block_offset in snapshot_set['block'].list_next_block_offset:
                if next_block_offset == 0x0:
                    ...
                    if not dict_store_block[next_block_offset].flag_dummy:
                        ...
                        store_block.data = (
                                                       original_data_block_offset + relative_store_data_block_offset + store_data_block_offset + flags + allocation_bitmap) * (
                                                       (0x4000 - 128) // 32)

                    store_block.relative_block_offset = store_file_offset
                    store_block.current_block_offset = store_file_offset
                    if dict_store_block[
                        next_block_offset].next_block_offset != 0 and store_block.next_block_offset != 0x0:
                        store_block.next_block_offset = store_file_offset + 0x4000
                    f_store.write(store_block)
                    store_file_offset = store_file_offset + 0x4000
...
def write_catalog(catalog_file, list_disk_catalog_entry, list_snapshot_set, catalog0x03, volume_size):
    ...
    if len(list_disk_catalog_entry) > 0:
        ...
        creation_time = epoch_as_filetime + (
                    timegm([now.year, now.month, now.day, now.hour, now.minute, now.second]) * hundreds_of_nanoseconds)
...
def main():
    parser = argparse.ArgumentParser(prog='vss_carver',
                                     description="Carve and rebuild VSS snapshot catalog and store from disk image.")
    ...
    if catalog_offset:
        dict_disk_catalog_entry, list_disk_catalog_entry = read_catalog_from_disk_image(disk_image, args.offset,
                                                                                        catalog_offset)
    else:
    ...
    catalog0x03 = write_store(args.store, list_disk_catalog_entry, dict_store_block, list_snapshot_set, disk_image,
                              args.offset)
